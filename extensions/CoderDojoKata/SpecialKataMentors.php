	...
	function getQuerySubCategories($parentCategory) {
		/*
			SELECT
				catlink.*, cat.*, kpage.*
			FROM
				`kata-categorylinks` as catlink
				inner join
				`kata-page` as kpage
				on catlink.cl_from = kpage.page_id
				join `kata-category` as cat
				on kpage.page_title = cat.cat_title
				where catlink.cl_to = 'KataMentors'
				and catlink.cl_type = 'subcat'
		*/
		return array (
			// 'options' => 'DISTINCT',
			'fields' => array ( 'catlink.*', 'cat.*', 'kpage.*' ),
			'tables' => array ( 'catlink' => 'categorylinks', 'kpage' => 'page', 'cat' => 'category' ),
			// default for page_namespace is all content namespaces (if requestedNamespace is false)
			// otherwise, page_namespace is requestedNamespace
			'join_conds' => array (
				'kpage' => array (
					'INNER JOIN', 'catlink.cl_from = kpage.page_id'
				),
				'cat' => array (
					'LEFT JOIN', 'kpage.page_title = cat.cat_title' )
			),
			'conds' => array ( "catlink.cl_to = '$parentCategory'", "catlink.cl_type = 'subcat'"),
		);

	}


	function getQuerySoftwares($letter) {
